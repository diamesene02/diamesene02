...
**Cette constitution prime sur toute autre pratique**, y compris sur les
gabarits de `.specify/templates/`. Une spec qui contredit un article est
refusée — ou amende la constitution, et alors elle porte l'amendement dans son
propre texte, avec la raison et la date.

**Amender** demande trois choses, dans cet ordre : le défaut réel qui l'exige
(un article sans défaut sous lui est une opinion), ce que l'amendement change
pour le club un lundi soir, et la mise à jour des specs déjà écrites qu'il
invalide.

**Vérification.** `/speckit-analyze` relit chaque lot contre ces articles avant
`/speckit-implement`. `/analyser NNNN` fait la même chose pour les specs
écrites à la main, en français, et va plus loin : elle rouvre chaque
`fichier:ligne` cité.

**Ce que la constitution ne fait pas.** Elle ne décrit pas le produit — c'est
`specs/0000-le-club-et-lapp/` (549 cas). Elle ne décrit pas la méthode — c'est
`specs/COMMENT-ON-TRAVAILLE.md`. Elle dit seulement ce qui ne se négocie pas.

**Version** : 1.0.0 | **Ratifiée** : 2026-09-10 | **Dernier amendement** : 2026-09-10

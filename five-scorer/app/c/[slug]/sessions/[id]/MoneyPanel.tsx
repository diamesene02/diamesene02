      ...
      {canManage ? (
        <div className="flex flex-wrap items-end gap-2">
          <label className="block">
            <span className="mb-1 block text-[11px] font-black uppercase tracking-widest text-[color:var(--ink-2)]">
              Prix du terrain (€)
            </span>
            <input
              type="text"
              ...
              placeholder="ex. 80"
              className="w-32 rounded-xl border border-[color:var(--stroke)] bg-[color:var(--bg-2)] px-3 py-2.5 text-sm font-bold tabular-nums outline-none focus:border-[color:var(--lime)]"
            />
          </label>
          <button
            disabled={pending}
            onClick={saveCost}
            className="big-touch rounded-xl border border-[color:var(--stroke-hi)] bg-[color:var(--bg-2)] px-4 py-2.5 text-sm font-bold transition-colors hover:border-[color:var(--lime)] disabled:opacity-50"
          >
            {pending ? "…" : "Enregistrer"}
          </button>
        </div>
      ...
      {cost != null && (
        <>
          ...
          <p className="mt-4 text-sm text-[color:var(--ink-1)]">
            {nbIn > 0 ? (
              <>
                <span className="font-black tabular-nums text-[color:var(--a-400)]">
                  {nbIn}
                </span>{" "}
                présent{nbIn > 1 ? "s" : ""} →{" "}
                <span className="num-sculpt text-xl text-[color:var(--lime)]">
                  {fmtEuro(shareCents!)}
                </span>{" "}
                chacun
              </>
          ...
          {/* Qui a payé */}
          {nbIn > 0 && (
            <ul className="mt-3 divide-y divide-[color:var(--stroke)] overflow-hidden rounded-2xl border border-[color:var(--stroke)] bg-[color:var(--bg-2)]">
              {rows.map((r) => (
                <li key={r.playerId}>
                  <label
                    className={cn(
          ...
          <div className="mt-4">
            ...
            <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-[color:var(--bg-2)]">
              <div
                className="h-full rounded-full bg-[color:var(--lime)] transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>

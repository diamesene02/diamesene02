...
| « C'est moi » — la route | **routine**, après un aller-retour | Elle accepte un `userId` dans le corps quand on gère le club. Ç'a d'abord été fermé ici, au motif qu'un gérant peut poser son compte sur la fiche d'un coéquipier — la transaction délie la sienne, l'autre perd la sienne sans un mot. **Puis rouvert, parce que la fermeture coûtait plus cher que le risque** : c'est le SEUL recours quand quelqu'un se trompe de fiche. Rien ne délie un profil — les deux seuls `userId: null` du dépôt sont des exclusions du club. Fermé, une revendication ratée le premier soir ne se répare qu'en expulsant la personne. C'est `parcours-lecture.mjs` qui l'a dit : son ménage passe par cet arbitrage, et il est tombé en panne. Ce qui manque n'est pas une garde de plus, c'est une **trace** — noté pour la spec produit. |
